from brownie import accounts, KhronusCoordinator, EscrowInfrastructure, BasicClient, KhronToken, TestKhronusNode, MockOracle, KhronPriceOracle;
# Testing utilities
...
def khron_constants_client():
    ...
    ethOracle = MockOracle.deploy(5000*1e8, {"from":khron_owner})
    maticOracle = MockOracle.deploy(2*1e8, {"from":khron_owner})
    khronOracle = KhronPriceOracle.deploy(ethOracle.address, maticOracle.address, {"from":khron_owner})
...
def khron_constants_operations():
    total_client_tokens = 100*10**18
    registration_deposit = 1*10**18
    tolerance_band = 5
    khron_owner = accounts[0]
    ...
    maticOracle = MockOracle.deploy(2*1e8, {"from":khron_owner})
    khronOracle = KhronPriceOracle.deploy(ethOracle.address, maticOracle.address, {"from":khron_owner})
    coordinator_contract = KhronusCoordinator.deploy(token_contract.address, khronOracle.address, registration_deposit, tolerance_band, {'from':khron_owner})
    client_contract = EscrowInfrastructure.deploy(coordinator_contract.address,{'from':client_owner})
    benchmark_client = BasicClient.deploy(coordinator_contract.address,{'from':client_owner})
    node_contract_0 = TestKhronusNode.deploy(coordinator_contract.address, {'from':node_owner_0})
    node_contract_1 = TestKhronusNode.deploy(coordinator_contract.address, {'from':node_owner_1})
    token_contract.transfer(client_owner.address, total_client_tokens,{'from':khron_owner})
    ...
    token_contract.increaseApproval(coordinator_contract.address, registration_deposit * 2, {'from':client_owner})
    coordinator_contract.registerClient(client_contract.address, registration_deposit, {'from':client_owner})
    coordinator_contract.registerClient(benchmark_client.address, registration_deposit, {'from':client_owner})
